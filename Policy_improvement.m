function [pol_pi,emp_pol_pi,policyStable] = Policy_improvement(pol_pi,emp_pol_pi,V,gamma,Ra,Pa,Rb,Pb,max_num_cars_can_transfer,max_cars_can_store)
...
for si=1:nStates, 
    ...
    b = pol_pi(na1,nb1);         %policy to bring back in when there are na and nb cars present at a and b site
    b_emp = emp_pol_pi(na1,nb1); %policy to empty out when there are na and nb cars present at a and b site

    % what are the any BETTER actions for this state?
    %
    ...
    % 
    % Assume the number of cars we can transfer is restricted only by the number we have
    % 
    %---
    % based on the state and action, compute the expectation over all possible states we may transition to i.e. s'
    %
    ...
    posActionsInState0 = [ -posB:posA ]; 
    npa = length(posActionsInState0); 
    Q0 = [];
    
    tic; 
    for ti = 1:npa,
      ntrans = posActionsInState0(ti);   
      Q0(end+1) = rhs_state_value_bellman(na,nb,ntrans,useEmp,V,gamma,Ra,Pa,Rb,Pb,max_num_cars_can_transfer,max_cars_can_store);
    end
    ...
    posActionsInState1 = [ -posB:posA ];
    npa = length(posActionsInState1);
    Q1 = []; 
    
    for ti = 1:npa,
      ntrans = posActionsInState1(ti);   
      Q1(end+1) = rhs_state_value_bellman(na,nb,ntrans,useEmp,V,gamma,Ra,Pa,Rb,Pb,max_num_cars_can_transfer,max_cars_can_store);
    end
    tm=toc; 
    
    % check if this policy gives the best action
    [max0,imax0] = max( Q0 ); %[C,I] = max(...) finds the indices of the maximum value C, and returns them in output vector I. 
    [max1,imax1] = max( Q1 ); 
    [dum,useEmpMax] = max( [max0,max1] ); %dum could be max0 or max1, useEmpMax could be 1 or 2
    useEmpMax=useEmpMax-1;  %useEmpMax could be 0 or 1
