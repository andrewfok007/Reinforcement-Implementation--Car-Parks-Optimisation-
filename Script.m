...
% the parameters of the environment - possion!
if( 0 ) 
...
emp_pol_pi = zeros(max_n_cars+1,max_n_cars+1); 
...
while( ~policyStable )
  ...
  if( iterNum~=0 ) 
    %if( 0 ) 
      ...
      drawnow;
      fn=sprintf('policy_iter_both_%d.eps',iterNum); 
      %saveas( gcf, fn, 'eps2' ); 
    %else
      figure;
      ...
      %title( ['current policy iter=', num2str(iterNum)] ); 
      drawnow;
      fn=sprintf('policy_iter_combined_%d.eps',iterNum); 
      %saveas( gcf, fn, 'eps2' ); 
    %end
  end
  
  % evaluate the state-value function under this policy: 
  V = Policy_evaluation(V,pol_pi,emp_pol_pi,gamma,Ra,Pa,Rb,Pb,max_num_cars_can_transfer,max_cars_can_store);
  ...
  if( 1 ) 
    ...
    fn=sprintf('state_value_fn_iter_%d.eps',iterNum); 
    %saveas( gcf, fn, 'eps2' ); 
  end
  
  % compute an improved policy using the most recent as a base: 
  [pol_pi,emp_pol_pi,policyStable] = Policy_improvement(pol_pi,emp_pol_pi,V,gamma,Ra,Pa,Rb,Pb,max_num_cars_can_transfer,max_cars_can_store);
